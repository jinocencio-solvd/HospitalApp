...
public class App {
    ...
    public static void designPatternsDemo() {
        // initialize state
        DaoType appDaoType = AppConfig.appDaoType;
        PatientService patientService = new PatientService(appDaoType);
        Patient patient = patientService.getPatientByPersonId(1);

        // set up MVC components
        DecoratedPatient dPatient = DecoratedPatientController.getDecoratedPatient(patient);
        DecoratedPatientView decoratedPatientView = new DecoratedPatientView();
        DecoratedPatientController decoratedPatientController = new DecoratedPatientController(
            dPatient, decoratedPatientView);

        // use controller
        decoratedPatientController.generateJsonApiView();
        decoratedPatientController.generateSimplePatientMedicalRecordsSysOutView();
    }
